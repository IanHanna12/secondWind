...
/// The XPC interface exposed by the optional root helper. Every operation is
/// typed; it intentionally has no command, argument, or shell-text parameter.
...
/// A deliberately narrow root-side operation. The helper accepts only one app
/// bundle directly inside `/Applications`, then moves it to the calling user's
/// Trash. It does not accept arbitrary filesystem locations.
public struct PrivilegedApplicationTrashRequest: Codable, Sendable {
    public let applicationPath: String
    public let bundleIdentifier: String

    public init(applicationPath: String, bundleIdentifier: String) {
        self.applicationPath = applicationPath
        self.bundleIdentifier = bundleIdentifier
    }
}

public struct PrivilegedApplicationTrashReply: Codable, Sendable {
...
public enum PrivilegedApplicationTrashError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .helperUnavailable: return "The optional privileged helper is unavailable. Enable it in System tasks to move this administrator-owned app to Trash."
        case .malformedReply: return "The privileged helper returned an invalid app-removal response."
        case .connectionInterrupted: return "The privileged helper connection was interrupted while moving the app to Trash."
        }
    }
}
...
/// App-side client for the helper's tightly scoped administrator-owned app
/// removal operation. It is used only after Finder-equivalent Trash movement
/// has been denied by macOS.
public final class XPCPrivilegedApplicationTrashClient: @unchecked Sendable {
    private let helperConnection: PrivilegedHelperConnection

    public init(serviceName: String = XPCPrivilegedMaintenanceClient.serviceName) {
        self.helperConnection = PrivilegedHelperConnection {
            NSXPCPrivilegedHelperTransport(serviceName: serviceName)
        }
    }

    public func moveApplicationToTrash(at applicationURL: URL, bundleIdentifier: String) async throws -> URL {
        let request = PrivilegedApplicationTrashRequest(
        ...
        let replyData: Data
        do {
            replyData = try await helperConnection.sendApplicationTrash(requestData)
        } catch let error as PrivilegedMaintenanceClientError {
            switch error {
            case .connectionInterrupted:
                throw PrivilegedApplicationTrashError.connectionInterrupted
            default:
                throw PrivilegedApplicationTrashError.helperUnavailable
            }
        }
        guard let reply = try? JSONDecoder.secondWind.decode(PrivilegedApplicationTrashReply.self, from: replyData) else {
            throw PrivilegedApplicationTrashError.malformedReply
        }
        return URL(fileURLWithPath: reply.trashPath)
    }
}
...
protocol PrivilegedHelperTransport: Sendable {
    ...
    func sendApplicationTrash(_ requestData: Data, reply: @escaping @Sendable (Result<Data, Error>) -> Void)
    func invalidate()
}

extension PrivilegedHelperTransport {
    func sendApplicationTrash(_ requestData: Data, reply: @escaping @Sendable (Result<Data, Error>) -> Void) {
...
private struct PrivilegedHelperConnection: Sendable {
    ...
    func send(_ requestData: Data) async throws -> Data {
        try await send(requestData) { transport, data, reply in
            transport.send(data, reply: reply)
        }
    }

    func sendApplicationTrash(_ requestData: Data) async throws -> Data {
        try await send(requestData) { transport, data, reply in
            transport.sendApplicationTrash(data, reply: reply)
        }
    }

    private func send(
        _ requestData: Data,
        using operation: @escaping @Sendable (any PrivilegedHelperTransport, Data, @escaping @Sendable (Result<Data, Error>) -> Void) -> Void
    ) async throws -> Data {
        ...
        return try await withTaskCancellationHandler(operation: {
            try await withCheckedThrowingContinuation { continuation in
                ...
                operation(transport, requestData) { outcome in
...
private final class NSXPCPrivilegedHelperTransport: @unchecked Sendable, PrivilegedHelperTransport {
    ...
    func send(_ requestData: Data, reply: @escaping @Sendable (Result<Data, Error>) -> Void) {
        send(requestData, reply: reply) { helper, data, response in
            helper.perform(data, withReply: response)
        }
    }

    func sendApplicationTrash(_ requestData: Data, reply: @escaping @Sendable (Result<Data, Error>) -> Void) {
        send(requestData, reply: reply) { helper, data, response in
            helper.moveApplicationToTrash(data, withReply: response)
        }
    }
    ...
    private func send(
        ...
        invoke: @escaping (PrivilegedMaintenanceXPC, Data, @escaping (Data?, NSError?) -> Void) -> Void
    ) {
        ...
        invoke(helper, requestData) { replyData, error in
            if let error {
                reply(.failure(error))
