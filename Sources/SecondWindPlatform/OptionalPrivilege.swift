...
public struct OptionalPrivilegeDetector: Sendable {
    ...
    public func detect(bundle: Bundle = .main) -> OptionalPrivilegeStatus {
        ...
        let plist = bundle.bundleURL.appendingPathComponent("Contents/Library/LaunchDaemons/\(Self.helperPlistName)")
        guard FileManager.default.fileExists(atPath: plist.path) else { return .init(signingIdentity: identity, helperState: .notBundled) }
...
public enum OptionalHelperRegistrationError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .notBundled: return "This app build does not include the optional privileged helper. Build the PrivilegedHelper Xcode scheme first."
        }
    }
}
...
public struct OptionalHelperRegistration: Sendable {
    ...
    public func register(bundle: Bundle = .main) throws {
        let plist = bundle.bundleURL.appendingPathComponent("Contents/Library/LaunchDaemons/\(OptionalPrivilegeDetector.helperPlistName)")
        guard FileManager.default.fileExists(atPath: plist.path) else { throw OptionalHelperRegistrationError.notBundled }
