...
public enum MaintenanceValidationError: LocalizedError { case remoteVolume, missingUUID, unsupported; public var errorDescription: String? { switch self { case .remoteVolume: return "Only a local volume may be selected."; case .missingUUID: return "The selected volume has no stable identifier."; case .unsupported: return "This task is not supported by the privileged helper." } } }
public struct MaintenancePreflight: Sendable { public init() {}; public func validate(task: MaintenanceTask, volume: VolumeReference?) throws { switch task { case .periodicScripts: guard task.isAvailable else { throw MaintenanceRunnerError.rejected("This macOS version does not provide manual periodic scripts.") }; case .rebuildSpotlightIndex, .verifyVolume: guard let volume else { throw MaintenanceValidationError.missingUUID }; guard volume.isLocal else { throw MaintenanceValidationError.remoteVolume }; guard volume.uuid != nil else { throw MaintenanceValidationError.missingUUID } } } }
