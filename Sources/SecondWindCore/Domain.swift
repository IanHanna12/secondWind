...
public struct PlanAction: Codable, Hashable, Identifiable, Sendable {
    ...
    public init(id: UUID = UUID(), findingID: UUID, title: String, sourcePath: String, byteSize: Int64, risk: Risk, action: SupportedAction, confidence: MatchConfidence) {
        self.id = id; self.findingID = findingID; self.title = title; self.sourcePath = sourcePath; self.byteSize = byteSize; self.risk = risk; self.action = action; self.confidence = confidence
    }
}
