...
public struct StorageInventoryEntry: Hashable, Identifiable, Sendable {
    ...
    /// Creates the inventory projection for a Recovery item. The caller
    /// supplies physical bytes because the manifest intentionally stores
    /// logical content size for integrity verification.
    public init(_ recoveryItem: RecoveryItem, allocatedByteSize: Int64) {
        key = "recovery|\(recoveryItem.id.uuidString)"
        title = "Recovery: \(LocalPathDisplay.name(for: recoveryItem.originalPath))"
