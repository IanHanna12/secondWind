...
public struct RuleEngine: Sendable {
    ...
    public func scan(progress: @Sendable (ScanProgress) -> Bool) -> ScanOutcome {
        ...
        findings += candidateFiles(in: home.appendingPathComponent("Downloads"))

        guard progress(.init(completedUnits: rules.count + 1, totalUnits: totalUnits, currentTitle: "Desktop")) else {
            return .cancelled
        }
        findings += candidateFiles(in: home.appendingPathComponent("Desktop"))
    ...
    private func candidateFiles(in root: URL) -> [Finding] {
