...
public struct PlanBuilder: Sendable {
    ...
    public func makePlan(findings: [Finding], selectedIDs: Set<UUID>, destination: PlanDestination) throws -> CleanupPlan {
        ...
        let actions = try selected.map { finding -> PlanAction in
            ...
            guard isFindingAllowed(finding, for: destination) else { throw PlanError.destinationNotAllowed(finding.path) }
            if finding.confidence == .needsUserReview { warnings.append("Review required: \(finding.path)") }
            return PlanAction(findingID: finding.id, ruleID: finding.ruleID, ruleVersion: finding.ruleVersion, title: finding.title, sourcePath: finding.path, byteSize: finding.byteSize, risk: finding.risk, action: finding.supportedAction, confidence: finding.confidence)
        }
    ...
    public func validate(_ plan: CleanupPlan, requireConfirmation: Bool = true) throws {
        ...
        for action in plan.actions {
            ...
            let sourcePath = URL(fileURLWithPath: action.sourcePath).standardizedFileURL.path
            guard isUnderAllowedRoot(sourcePath, action: action.action, destination: plan.destination) else { throw PlanError.invalidPath(sourcePath) }
        }
    }

    private func isFindingAllowed(_ finding: Finding, for destination: PlanDestination) -> Bool {
        let sourcePath = URL(fileURLWithPath: finding.path).standardizedFileURL.path
        return isUnderAllowedRoot(sourcePath, action: finding.supportedAction, destination: destination)
    }
    ...
    public func execute(_ plan: CleanupPlan) async throws -> [ExecutionResult] {
        ...
        for action in plan.actions {
            do {
                ...
                case .finderTrash: try await trashMover.moveToTrash(url); result = .trashed(action.sourcePath)
