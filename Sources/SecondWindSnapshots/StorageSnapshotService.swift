...
public struct StorageSnapshotService: Sendable {
    ...
    public func capture(findings: [Finding], totalBytes: Int64, availableBytes: Int64, at date: Date = Date()) -> StorageSnapshot {
        let entries = findings.map { finding in
            StorageSnapshotEntry(
                ...
                title: finding.title,
                category: category(for: finding.ruleID),
    ...
    private func category(for ruleID: String) -> String {
        switch ruleID {
        case "xcode-derived-data", "xcode-archives", "simulator-devices", "gradle-cache": return "Developer"
        case "npm-cache", "homebrew-cache": return "Package managers"
        case "user-logs": return "Logs"
        case "downloaded-installer": return "Installers"
        case "large-personal-file": return "Large personal files"
        case "docker-data": return "Containers"
        case "browser-data": return "Browser data"
        case "application-inventory", "application-remnant": return "Applications"
