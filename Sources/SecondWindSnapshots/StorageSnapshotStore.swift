...
public final class StorageSnapshotStore: @unchecked Sendable {
    ...
    public init(url: URL? = nil, fileManager: FileManager = .default) {
        ...
        self.url = url ?? fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("SecondWind/StorageStory/snapshots.json")
    }

    public func snapshots() -> [StorageSnapshot] {
        guard let data = try? Data(contentsOf: url), let snapshots = try? JSONDecoder.secondWind.decode([StorageSnapshot].self, from: data) else { return [] }
    ...
    public func append(_ snapshot: StorageSnapshot) throws -> [StorageSnapshot] {
        ...
        try JSONEncoder.secondWind.encode(values).write(to: url, options: .atomic)
        return values
    }
}
