...
func verify() throws {
    ...
    let audit = AuditStore(url: root.appendingPathComponent("audit.jsonl")); try audit.append(.init(kind: .scan, ruleVersions: ["user-logs v1"], paths: ["/tmp/log"], bytes: 1, result: "ok")); try audit.append(.init(kind: .dryRun, ruleVersions: [], paths: [], bytes: 0, result: "ok")); try check(audit.records().count == 2, "append-only audit"); try check(audit.exportMarkdown().contains("dryRun"), "audit markdown export")
}
